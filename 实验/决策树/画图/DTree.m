function [Tree RulesMatrix]=DTree(DataSet,AttributName)
...
function showTree(Tree,level,value,branch,AttributValue,AttributName)
    ...
    if(length(Tree.Child)~=0) %非叶子节点
        if isempty(AttributName)
             disp([blank  int2str(Tree.Attribut)]);
        else
             disp([blank  AttributName{Tree.Attribut}]);
        end
        if isempty(AttributValue)
        ...
        switch Tree.Attribut
            case 1
                attr = 'Hard contact lense';
            case 2
                attr = 'Soft contact lense';
            case 3
                attr = 'No contact lense';
            otherwise
                attr = int2str(Tree.Attribut);
        end
       if isempty(AttributValue)
           
           disp([blank ' ' attr]);
       else
           disp([blank ' ' AttributValue{length(AttributValue)}{attr}]);
       end
    end     
end
